...
original queued scope (an interactive gold/NAV lookup tool, ported from
Family Ledger, living as its own `/tools` entry) is replaced entirely by
a quiet, passive, site-wide info banner -- current USD/INR rate and gold
price, visible on every page (calculators, blog, FAQ, home), not a
separate tool page, not interactive, no links out. See ADR 0021 for the
full reasoning; `state.json`'s own `nric-011` entry was rewritten to
match before work started, not left with the stale "lookup tool"
wording.

New: `src/components/PriceBanner.tsx`, an `async` Server Component
(deliberately not a client component fetching from a dedicated API
route, unlike `nric-012`'s tool) wired directly into `src/app/layout.tsx`
between the nav and `<main>`. Being a Server Component means there is no
client-side loading state at all -- the data is already resolved before
any HTML reaches the browser, a better fit for "never show a spinner for
a passive strip" than a client-fetched design could achieve.

Reuses rather than rebuilds: factored a shared `FRANKFURTER_BASE`
constant out of `nric-012`'s `/api/fxhistory` route into a new
`src/lib/fx.ts`, which also gets a lean `fetchCurrentUsdInrRate()`
hitting frankfurter's `/latest` endpoint directly (a full timeseries
fetch just to read one number would be real, avoidable waste at
site-wide call volume). Gold pricing (`src/lib/gold.ts`) mirrors Family
Ledger's own `api/metals.js` rather than re-deriving the math -- goldapi.io
already returns `price_gram_24k` directly, so the only conversion needed
here is USD-per-gram to INR-per-10g via the already-fetched FX rate.
`GOLD_API_KEY` confirmed as the exact env var name against the actual
Vercel dashboard (Production + Preview), not just inferred from Family
Ledger's matching convention.

Both fetches cache for 1 hour (same judgment-call precedent as
`nric-012` -- ECB/goldapi publish infrequently, and this now runs on
every pageview site-wide, so protecting GoldAPI's free-tier quota
matters much more here) and carry a 3-second timeout each so a hanging
upstream can't drag down every page's render.

**Fail-quiet, with a deliberate coupling**: gold is shown as an INR
figure, so it structurally depends on the FX rate too, not just its own
fetch succeeding -- if FX fails, the gold piece is omitted even when
goldapi itself responded fine, rather than falling back to an
unrequested raw-USD display. If both fail, the entire banner element is
omitted, not an empty shell. Verified both paths for real: this
environment has no local `GOLD_API_KEY`, so every local run already
exercises the genuine partial-failure path (confirmed via DOM inspection
-- only the USD/INR piece renders, no dangling separator); full failure
was verified by temporarily pointing the FX fetch at a non-resolving
host, confirming the banner element is completely absent with zero
console errors, then reverting and re-confirming a clean `git diff`
before continuing.

Mobile: the full combined string fits one line at 390px even at a
worst-case 6-digit gold price (verified via live screenshot), with a
shortened "Gold (24K)" label below the `sm:` breakpoint as extra margin.
Styled entirely with this site's existing muted `stone` neutrals (ADR
0004) -- no new color, no animation, no ticker, not a link.

**Cannot be end-to-end verified against live GoldAPI.io data without
deploying** -- no real key exists in this environment, same gap every
other keyed-API task on this site has flagged rather than worked around.
Verified instead against the documented goldapi.io response shape and
the real (not simulated) local fail-quiet path.

Verified: `npx tsc --noEmit` (clean), `npx eslint .` (clean), `npm run
build` (85 routes, unchanged from `nric-012` -- no new route added since
this went the direct-layout-fetch path; every route's build output now
shows an explicit `Revalidate: 1h` column, confirming the cache policy
applied sitewide while every page stayed statically prerendered), and a
live `npm run dev` smoke test across a calculator page, a blog article,
FAQ, and home, at both 1440px and 390px.

Added `docs/decisions/0021-site-wide-price-banner.md`.
